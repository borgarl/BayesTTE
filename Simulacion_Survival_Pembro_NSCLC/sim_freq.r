...
sim_freq <- function(n_sim = 1, 
  ...
  result_list <- apply(grid, 1, function(x) {
    ...
    df <- surv_analysis_freq(sample_size = sample_size,
    ...
    colnames(df) <- c("HR", "Lower_IC", "Upper_IC", "Reject", "p_value","Time_significant", "Comparison",
                       "MSE", "Coverage_Probability")
    #colnames(df) <- c("HR", "Upper_IC", "Lower_IC", "Reject", "p_value","p_value_weibull","Time_significant", "Comparison",
    #                  "MSE", "Coverage_Probability")
    
    df[, 1:5] <- lapply(df[, 1:5], as.numeric)
    df[, 8:9] <- lapply(df[, 8:9], as.numeric)
    # df[, 1:6] <- lapply(df[, 1:6], as.numeric)
    # df[, 9:10] <- lapply(df[, 9:10], as.numeric)
  ...
  analysis <- analysis[order(analysis$scenario, analysis$Comparison,
                             analysis$sample_size, analysis$n_sim),]
  
 # Crear un resumen de resultados
  
  summary_data <- analysis %>%
    group_by(scenario, sample_size, Comparison) %>%
    summarise(
      mean_HR = mean(HR, na.rm = TRUE),
      mean_Lower_IC = mean(Lower_IC, na.rm = TRUE),
      mean_Upper_IC = mean(Upper_IC, na.rm = TRUE),
      ...
      median_Lower_IC = median(Lower_IC, na.rm = TRUE),
      median_Upper_IC = median(Upper_IC, na.rm = TRUE),
      .groups = "drop")
  ...
  IA_counts <- analysis %>%
    group_by(scenario, sample_size, Comparison, Time_significant) %>%
    summarise(
      count = n(),
      prop = count / sum(count),
      .groups = "drop"
    ) %>%
    mutate(Time_significant = paste0("IA_", Time_significant)) %>% 
    pivot_wider(names_from = Time_significant, values_from = count, values_fill = 0) %>% 
  ...
  IA_counts <- IA_counts %>%
    select(scenario, sample_size, Comparison, all_of(IA_cols_sorted), all_of(FA_col))
  
  
  # Unir la info general con la de los IAs
  
  summary_data <- left_join(summary_data, IA_counts, by = c("scenario","sample_size","Comparison"))
  ...
  if(plot_pvalues == TRUE){
    ...
    pvalues <- analysis %>%
      select(scenario, sample_size, Comparison, n_sim, HR, Upper_IC, Lower_IC, Coverage_Probability, p_value)
    # select(scenario, sample_size, Comparison, n_sim, HR, Upper_IC, Lower_IC, Coverage_Probability, p_value, p_value_weibull)
  ...
  if(Plot_Power == TRUE & is.null(IA) & dim(scenarios_eff)[1] == 2){
    ...
    # Plot para dibujar el MSE a lo largo de todos los tamaños muestrales
